...
local Debris = game:GetService("Debris")  -- [attached_file:2]

-- Player refs
local player = Players.LocalPlayer
local PlayerGui = player:WaitForChild("PlayerGui")  -- [attached_file:2]
...
Blur.Enabled = true
Blur.Size = 0
Blur.Parent = camera  -- [attached_file:2]
...
ScreenGui.Parent = PlayerGui  -- [attached_file:2]
...
Root.Parent = ScreenGui  -- [attached_file:2]

local RootCorner = Instance.new("UICorner")
RootCorner.CornerRadius = UDim.new(0, 14)
RootCorner.Parent = Root  -- [attached_file:2]
...
RootStroke.Parent = Root  -- [attached_file:2]
...
Glow.Parent = Root  -- [attached_file:2]
...
Bg.Parent = Root  -- [attached_file:2]

local BgCorner = Instance.new("UICorner")
BgCorner.CornerRadius = UDim.new(0, 14)
BgCorner.Parent = Bg  -- [attached_file:2]
...
BgGradient.Parent = Bg  -- [attached_file:2]
...
CopyBtn.Parent = RightButtons  -- [attached_file:2]
...
CopyStroke.Parent = CopyBtn  -- [attached_file:2]
...
InfoBtn.Parent = Root  -- [attached_file:2]
...
InfoStroke.Parent = InfoBtn  -- [attached_file:2]
...
Status.Parent = Root  -- [attached_file:2]
...
StatusGlow.Parent = Root  -- [attached_file:2]
...
StatusGlowGrad.Parent = StatusGlow  -- [attached_file:2]
...
DiamondCountLabel.Parent = Root  -- [attached_file:2]
...
InfoTab.Parent = Root  -- [attached_file:2]
...
InfoBackdrop.Parent = InfoTab  -- [attached_file:2]
...
InfoBackdropStroke.Parent = InfoBackdrop  -- [attached_file:2]
...
InfoHeader.Parent = InfoBackdrop  -- [attached_file:2]
...
CloseInfo.Parent = InfoBackdrop  -- [attached_file:2]
...
CloseStroke.Parent = CloseInfo  -- [attached_file:2]
...
InfoBody.Parent = InfoBackdrop  -- [attached_file:2]
...
HiddenCopyBox.Text = ""
HiddenCopyBox.Parent = ScreenGui  -- [attached_file:2]
...
CloseInfo.MouseButton1Click:Connect(hideInfo)  -- [attached_file:2]
...
local TeleportService = game:GetService("TeleportService")
local HttpService = game:GetService("HttpService")
local Lighting = game:GetService("Lighting")  -- [attached_file:2]
...
local Debris = ui.Debris  -- [attached_file:2]

-- Remotes/containers (names from working version)
local RemoteEvents = ReplicatedStorage:WaitForChild("RemoteEvents")
local TakeDiamonds = RemoteEvents:WaitForChild("RequestTakeDiamonds")
local ItemsFolder = Workspace:WaitForChild("Items")  -- [attached_file:2]
...
local function ClearLag()
    ...
    local function shouldCullName(n)
        n = string.lower(n or ""); return n:find("decal") or n:find("particle") or n:find("vfx") or n:find("smoke") or n:find("sparks")
    end
    for _, d in ipairs(Workspace:GetDescendants()) do
        ...
        if shouldCullName(d.Name) then pcall(function() if d:IsA("ParticleEmitter") or d:IsA("Beam") then d.Enabled = false end end) end
    end
end  -- [attached_file:2]

-- Run lag clear at start if not lobby
task.spawn(function() if PlaceID ~= 79546208627805 then ClearLag() end end)  -- [attached_file:2]
...
task.spawn(setupDiamondCountListener)  -- [attached_file:2]
...
cleanupCharactersFolder()  -- [attached_file:2]
